...
import { addClassicToCart, addClassicToList } from "./redux/Actions";
...
class Classic extends Component {
    ...
    render() {
        return(
            <>  <h2 className='typeNames'>CLASSICS</h2>
            <div className="d-flex">
                ...
                <div className="kindofBooks">
                    {this.props.classic.map((obj,index)=>(
                        ...
                        <div className="listAndCart">
                            <button button onClick={()=> {
                                    this.props.addClassicToList(index);
                                }} >ADD TO LIST</button>
                            <button 
                            onClick={()=>{this.props.addClassicToCart(index)}}
                            >ADD TO CART</button>
                        </div>
                    </div>
                    ))}
...
const mapDispatchToProps=(dispatch) => {
    return {
        addClassicToCart: (index) => {
            dispatch(addClassicToCart(index));
        },
        addClassicToList: (index) =>  {
            dispatch(addClassicToList(index));
        },
    }
}
