...
import { addPoemToCart, addPoemToList } from "./redux/Actions";
...
class Poem extends Component {
    ...
    render() {
        return(
          <>
          <h2 className="typeNames">POEMS</h2>
                ...
                <div className="kindofBooks">
                    {this.props.poem.map((obj,index)=>(
                        ...
                        <div className="listAndCart">
                            <button  onClick={()=> {
                                    this.props.addPoemToList(index);
                                }} >ADD TO LIST</button>
                            <button onClick={()=>{this.props.addPoemToCart(index)} }>ADD TO CART</button>
                        </div>
                    </div>
                    ))}
...
const mapDispatchToProps=(dispatch)=> {
    return {
        addPoemToCart: (index) => {
            dispatch(addPoemToCart(index));
        },
        addPoemToList: (index) =>  {
            dispatch(addPoemToList(index));
        }
    }
